...
const ParticleField = () => {
  ...
  useFrame((state, delta) => {
    if (ref.current) {
      baseRot.current.x -= delta * 0.03;
      baseRot.current.y -= delta * 0.05;
      
      const targetRotX = baseRot.current.x + (state.pointer.y * 0.2);
      const targetRotY = baseRot.current.y + (state.pointer.x * 0.2);
...
const CyberBackground = () => {
  return (
    <div className="fixed inset-0 z-0 pointer-events-none bg-[#030303]">
      <Canvas eventSource={typeof window !== 'undefined' ? document.getElementById('root') : undefined} eventPrefix="client" camera={{ position: [0, 0, 1.5] }}>
        <ParticleField />
      </Canvas>
